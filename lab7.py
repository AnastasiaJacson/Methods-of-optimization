...
def penalty_function(x, c, f, g, p):
    ...
    calls += 1
    penalty = f(x) + c / 2 * sum([max(0, gi(x))**2  for gi in g])
    return penalty
...
def constrained_optimization(f, g, x0, c0, p, epsilon, optim_method, K):
    ...
    while True:
        ...
        if all([gi(x) <= 0 for gi in g]) and (x[0] > 0 and x[1] > 0) and (abs(prev - penalty_func(x))) < epsilon:
            break
...
class Optimization:
    def __init__(self, master):
        ...
        self.constraint1_str = tk.StringVar(master, value='2*x[0] + x[1] - 14')
    ...
    def run_optimization(self):
        ...
        try:
            ...
            g = [eval(f'lambda x: {constraint1_str}'), eval(f'lambda x: {constraint2_str}')]

            f = eval(f'lambda x: {function_str}')
            initial_point = eval(initial_point)
            epsilon = eval(precision)

            if method == 'Ньютонівський':
            ...
            messagebox.showerror("Error", f"Вибери {e.value_name}")
            return
